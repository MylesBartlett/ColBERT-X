...
def train(args):
    ...
    if args.distributed:
        ...
        print(
            "Using args.bsize =", args.bsize, "(per process) and args.accumsteps =", args.accumsteps
        )
    ...
    for batch_idx, BatchSteps in zip(range(start_batch_idx, args.maxsteps), reader):
        ...
        if is_main_process(args.rank):
            ...
            Run.log_metric(
                "train/examples", num_examples_seen, step=batch_idx, log_to_mlflow=log_to_mlflow
            )
            Run.log_metric(
                "train/throughput",
